...
interface UseWepinReturn extends WepinState {
    ...
    const initWepin = async () => {
      try {
        ...
        await sdk.init({
          ...
          defaultLanguage: "en",
          defaultCurrency: "USD",
          loginProviders: ["google", "apple"], // Only Google and Apple for now
        });
  ...
  const connect = useCallback(
    async (provider: "google" | "apple" | "naver" | "discord" = "google") => {
      ...
      try {
        ...
        const network = evmAccount.network || "evmvery";

        // Step 5: Get provider for the account's network
        const ethereumProvider = await wepinProvider.getProvider(network.toLowerCase());
